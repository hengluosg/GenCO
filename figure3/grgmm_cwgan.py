...
def train_cgan(X, thetas, z_dim, n_iterations, batch_size):
    ...
    # 定义优化器
    g_optimizer = optim.Adam(G.parameters(), lr=1e-6, betas=(0.5, 0.999))
...
def plot_theta_distribution1(x, y,z,x_new):
    ...
    for i in range(d):
        ...
        axes[i].hist(x[:, i], bins=100, color='blue', alpha=0.7, density=True,label='Random search-distribution')
    ...
    if d == 2:
        ...
        # 绘制真实的 θ 的散点图
        sns.scatterplot(x=x[:, 0], y=x[:, 1], color='blue', s=10, alpha=0.7, label='Random search')
        # 绘制生成的 θ 的散点图
        sns.scatterplot(x=y[:, 0], y=y[:, 1], color='red', s=10, alpha=0.7, label='Generate(QRGMM)')
        
        sns.scatterplot(x=z[:, 0], y=z[:, 1], color='#2ca02c', s=10, alpha=0.7, label='Generate(CWGAN)')
...
def compute_wasserstein(x, y, z):
    # x: random data, y: qrgmm data, z: cwgan data
    d  = x.shape[1]
    wasserstein_losses_qrgmm = []
    wasserstein_losses_cwgan = []

    for dim in range(d):
        wd_qrgmm = wasserstein_distance(
            x[:, dim], y[:, dim]
        )
        wd_cwgan = wasserstein_distance(
            x[:, dim], z[:, dim]
        )
        wasserstein_losses_qrgmm.append(wd_qrgmm)
        wasserstein_losses_cwgan.append(wd_cwgan)
        
    return wasserstein_losses_qrgmm, wasserstein_losses_cwgan


def plot_wasserstein_distance(data_qrgmm, data_cwgan):
    sns.set_theme()
    fig, axes = plt.subplots(theta_dim, 1, figsize=(8, 4 * theta_dim))
    if theta_dim == 1:
        axes = [axes]
    for col in range(theta_dim):
        ax = axes[col]
        ax.hist(data_qrgmm[:, col], bins=20, color='blue', alpha=0.5, label='Generate(QRGMM)', edgecolor='black')
        ax.hist(data_cwgan[:, col], bins=20, color='green', alpha=0.5, label='Generate(CWGAN)', edgecolor='black')
        ax.set_title(f"Wasserstein Distance: $\\theta_{{{col+1}}}$")
        ax.legend()
        ax.grid(True)
    ...
    plt.savefig('Wasserstein_dis.pdf', format='pdf')
    
    plt.close()


def plot_optimality_gap_distribution(data):
    ...
    sns.histplot(data, bins=100, color='blue', alpha=0.5, label='Generate(QRGMM)', edgecolor='black')
    #sns.histplot(data, bins=100, kde=True, color='blue', alpha=0.5)
    # kde_data = ax.get_lines()[0].get_data()
    # plt.fill_between(kde_data[0], kde_data[1], alpha=0.3, color='skyblue')
    
   
    # Set labels and title
    plt.xlabel(r'$\Delta(x)$', fontsize=12)
    #plt.ylabel("Frequency", fontsize=12)
    plt.title("Distribution of Optimality Gap", fontsize=14)
    
    # Add legend
    plt.legend()
    plt.grid(True)
    
    plt.savefig('optiality_gap.pdf', format='pdf')
...
if __name__ == '__main__':
    ...
    replication  = 100
    data_qrgmm = np.zeros((replication,theta_dim))
    data_cwgan = np.zeros((replication,theta_dim))
    x_new = np.random.uniform(low=lowbound, high=upbound, size=(1, covariate_dim))
    global_solution ,f_min= solver(x_new)
    print(f_min)
    
    
    optimality  = np.zeros(replication)
    ...
    ######### online application
        ...
        for i in range(M):
            ...
            clip_theta = project(theta_g.detach().cpu().numpy().squeeze(), lowbound, upbound)
            candidates.append(clip_theta)
        solutions_cwgan = np.array(candidates).reshape((len(candidates),theta_dim))
        ...
        # x: random data, y: qrgmm data, z: cwgan data
        dis_qrgmm, dis_cwgan = compute_wasserstein(solutions_distribution, solutions, solutions_cwgan)
        print('dis_qrgmm: ', np.array(dis_qrgmm), 'dis_cwgan: ', np.array(dis_cwgan))
        
        data_qrgmm[j,: ]  = np.array(dis_qrgmm)
        data_cwgan[j,: ]  = np.array(dis_cwgan)
        end_time = time.time()  
        ...
        f = objective_function(best_solution,x_new)
        optimality[j] = f - f_min
        print(optimality[j])


    np.save('data_qrgmm.npy', data_qrgmm)
    np.save('data_cwgan.npy', data_cwgan)
    np.save('optimality.npy', optimality)
    plot_wasserstein_distance(data_qrgmm, data_cwgan)
    plot_optimality_gap_distribution(optimality)
