...
def lr_surfacre_model(X, thetas,  objective_values, degree=2, use_sin_cos=True, alpha=1.0):

    n, K, theta_dim = thetas.shape
    X_repeated = np.repeat(X, K, axis=0)  # 重复 X 数据 K 次
    thetas_flat = thetas.reshape(-1, theta_dim)  # 展平 thetas
    X_train = np.hstack([thetas_flat, X_repeated])  # 拼接特征
    Y_train = objective_values.flatten()  # 展平目标变
    return X_train, Y_train 




def train_nn_surface_model(X, thetas, objective_values, x_dim, epochs=300, lr=1e-3, batch_size=16):
    # 检查是否有可用的 GPU
...
def train_model_for_tau(X, Y, tau, hidden_dim=64, lr=0.001, epochs=100, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        #print(f"Epoch [{epoch + 1}/{epochs}], Tau: {tau:.2f}, Loss: {epoch_loss / len(dataloader):.4f}")
...
class OfflineModel:
    ...
    def train_multi_output_qrgmm(self, X, Y, m, hidden_dim=64, lr=0.001, epochs=300, batch_size=32):
        ...
        all_models = []
        for l in range(d):
            print(f"Training models for target variable {l + 1}/{d}")
            results = ray.get([
...
def plot_theta_distribution1(x, y,x_new):
    ...
    plt.tight_layout()
    plt.savefig('distribution_rs2.pdf', format='pdf')
    plt.show()
    if d == 2:
        ...
        sns.scatterplot(x=x[:, 0], y=x[:, 1], color='blue', s=5, alpha=0.7, label='True (random search)')
        sns.scatterplot(x=y[:, 0], y=y[:, 1], color='red', s=5, alpha=0.7, label='Generate-(QRGMM)')
        plt.title('contour plots')
        plt.xlabel(r'$\theta_1$')
        plt.ylabel(r'$\theta_2$')
        plt.legend(title="Online Stage")
        plt.savefig('Scatter_rs2.pdf', format='pdf')
        plt.tight_layout()
        plt.show()
...
def solver(x):
    num_initial_points = 50* theta_dim  
...
if __name__ == '__main__':
    ...
    for theta_dim in theta_dim1:
        ...
        thetas,objective_values,best_theta = offline_model.random_search(covariates, K)
        print(covariates.shape, thetas.shape, objective_values.shape)
        n, K, theta_dim = thetas.shape
        X_repeated = np.repeat(covariates, K, axis=0)  
        thetas_flat = thetas.reshape(-1, theta_dim)  

        Y_train = objective_values.flatten()  
        
        # X_train, Y_train = lr_surfacre_model(covariates, thetas, objective_values)
    ...
    # for i in range(M):
    #     solutions_distribution[i] = offline_model.random_search(x_new, K)[2]
    ...
    # plt.grid(True)
    # plt.savefig('qrgmm_rs2.pdf', format='pdf')
    # plt.show()
