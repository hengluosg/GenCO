...
# class TransformerSurfaceModel(nn.Module):
#     def __init__(self, input_dim, d_model=128, nhead=4, num_encoder_layers=2):
#         super(TransformerSurfaceModel, self).__init__()
#         self.embedding = nn.Linear(input_dim, d_model)
#         self.positional_encoding = nn.Parameter(torch.zeros(1, 500, d_model))  # 假设最大序列长度为500
...
#         x = x.mean(dim=1)  # 平均池化，汇总序列信息
#         return self.fc_out(x)

# def train_nn_surface_model(X, thetas, objective_values, x_dim, epochs=300, lr=1e-2, batch_size=16):
#     n, theta_dim = thetas.shape
#     X_repeated = X  # 将 X 重复 K 次
#     thetas_flat = thetas.reshape(-1, theta_dim)  # 展平 thetas
#     inputs = np.hstack([thetas_flat, X_repeated])  # 拼接 theta 和 x
...
#     # 定义 Transformer 模型
#     model = TransformerSurfaceModel(input_dim=theta_dim + x_dim)
#     optimizer = torch.optim.Adam(model.parameters(), lr=lr)
#     criterion = nn.MSELoss()
...
#             # 为 Transformer 增加序列维度
#             batch_inputs = batch_inputs.unsqueeze(1)  # (batch_size, seq_len=1, input_dim)
...
class OfflineModel:
    ...
    def first_stage(self, X, K):
        ...
        for x in X:
            ...
            best_theta = solutions[np.argmin([objective_function(theta, x) for theta in solutions])]  
...
class OnlineApplication:
    ...
    def select_best_solution(self, x, solutions):
        ...
        for solution in solutions:
            ...
            #score =objective_function(solution, x.flatten())
...
def plot_theta_distribution_offline(x, y,x_new):
    K, d = y.shape  
    sns.set_theme()
    if d == 1:
...
if __name__ == '__main__':
    ...
    objective_values, thetas = offline_model.first_stage(covariates, K)
    #pic_dis(thetas)
    ...
    solutions_distribution_df = pd.DataFrame(solutions_distribution)
    solutions_df = pd.DataFrame(solutions)
    solutions_df.to_csv('./solutions.csv', index=False)
    solutions_distribution_df.to_csv('./true_solutions.csv', index=False)
