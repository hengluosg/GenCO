...
if __name__ == '__main__':
    ...
    for t ,theta_dim in enumerate(theta_dim1):
        ...
        x_new = x_new.reshape(1,covariate_dim)
        for i in range(M):
            solutions_distribution[i] = offline_model.first_stage(x_new, K)[2]
    ...
    np.save('Generative_10_solutiond(QRGMM).npy', mse_method4_d)
    df = pd.DataFrame({
    ...
    plt.grid(True)
    plt.savefig('qrgmm_multi.pdf', format='pdf')
    plt.show()
